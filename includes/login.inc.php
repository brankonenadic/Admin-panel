...
$check = new UserClass;
 ...
 if (isset($_POST['checkLoginEmail'])) {
    ...
    if (isset($_POST['login'])) {
        ...
        $chackLoginPassword= $check->checkPassword($loginEmail, $loginPassword);
     if ($chackLoginPassword == false) {
        $error =["valid" => false,"msg" => "*Wrong email or password !"];
        echo json_encode($error);
     } else {
        $login = $check->login($loginEmail, $loginPassword);
        if ($login == false) {
            $error =["valid" => false,"msg" => "*Error login new !"];
            echo json_encode($error);
         } else { 
